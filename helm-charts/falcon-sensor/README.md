...
> [!WARNING]
> **`node.backend` is deprecated.** Setting this field has no effect — the sensor automatically
> uses eBPF mode by default and falls back to kernel mode on unsupported kernel versions. The field
> will be removed in a future release. If you currently set `node.backend: kernel` in your values,
> remove it; the sensor will continue to function correctly without it.
...
| `node.backend`                  | **(Deprecated)** This field is ignored. The sensor uses eBPF by default, but falls back to kernel mode on unsupported kernel versions. This option may be removed in a future sensor release.                                                                                                        | bpf                                                                                                                        |
...
| `container.podLabels`                         | Additional labels to add to container sidecar Deployment pod metadata. Note: may affect WorkloadAllowlists in GKE Autopilot. Example: `azure.workload.identity/use: "true"`. | `{}`                                                                                                                                                                                                                                                 |
